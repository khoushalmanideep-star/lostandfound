...
import { type LostItemFormValues, lostItemSchema } from "@/features/items/schemas/lost-item";
...
export function ReportLostItemForm() {
  ...
  const onSubmit = async (values: LostItemFormValues) => {
    ...
    if (!response.ok) {
      ...
      toast.error(payload?.error ?? "Could not submit lost item.");
      return;
    }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 md:grid-cols-2">
      ...
      <div className="space-y-1 md:col-span-1">
        ...
        <input
          ...
          {...register("category")}
          className="w-full rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900"
          placeholder="Bag, Electronics, ID Card"
        />
      ...
      <div className="space-y-1 md:col-span-1">
        ...
        <input
          ...
          className="w-full rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900"
        />
        {errors.dateLost ? <p className="text-xs text-red-600">{errors.dateLost.message}</p> : null}
      </div>
      ...
      <div className="space-y-1 md:col-span-2">
        ...
        <textarea
          ...
          className="w-full rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900"
          placeholder="Describe identifying marks, brand, and contents."
        />
